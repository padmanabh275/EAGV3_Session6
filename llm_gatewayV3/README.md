...
V3 is V2 with a **router pool** in front. Same MCP server and agentic code paths, now with eight worker providers. The new thing is a separate set of small/fast LLMs whose only job is to classify each incoming request and decide which tier of worker should handle it.

| Tier | Estimated tokens | Worker order |
|---|---|---|
| **TINY**  | < 1,000 | github → openai → openrouter → groq → nvidia → cerebras → gemini → ollama |
| **LARGE** | 1,000 – 8,000 | gemini → groq → nvidia → cerebras → openai → github → openrouter → ollama |
...
Worker provider set with current defaults:
...
| `oa`, `oai` | OpenAI | `gpt-4.1-mini` | account-dependent |
...
Failover order (worker pool) is configurable via `LLM_ORDER` env. Default: `ollama,gemini,nvidia,groq,cerebras,openrouter,openai,github`.
...
LLM_ORDER=ollama,gemini,nvidia,groq,cerebras,openrouter,openai,github
...
- **No router-pool expansion vs V2.** Mistral AI and OVHcloud were evaluated and parked for V4. Router pool stays at 4 providers; OpenAI is available as a worker provider in the failover ring.
